...
for j in range(0,20):
	for i in range(0, 20):
		if(i < 17):
			if Matrix[j][i]*Matrix[j][i+1]*Matrix[j][i+2]*Matrix[j][i+3] > maximum:
				maximum = Matrix[j][i]*Matrix[j][i+1]*Matrix[j][i+2]*Matrix[j][i+3]
		if(j < 17):
			if Matrix[j][i]*Matrix[j+1][i]*Matrix[j+2][i]*Matrix[j+3][i] > maximum:
				maximum = Matrix[j][i]*Matrix[j+1][i]*Matrix[j+2][i]*Matrix[j+3][i]
		if(j < 17 and i < 17):
